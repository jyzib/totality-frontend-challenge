    ...
    <div className=" mt-20 flex flex-col md:flex-row">
      <div className="flex flex-col md:w-1/2">
        <div className="max-w-md">
          ...
          <div className="pt-10">
            <h1 className="text-4xl font-bold tracking-wide">{property[0]?.title}</h1>
          </div>
          <div className="flex items-center justify-between pt-4">
            ...
            <div className="flex items-center">
              <div className="flex gap-2 space-x-px">
                {[...Array(5)].map((_, index) => (
                  <div key={index} className="w-5 h-5 rounded-full "><Star className='text-white' fill='#FFC41F' /></div>
                ))}
              </div>
              <div className="pl-2 leading-none">4.6 / 5.0 <span className="text-gray-900/40">(556)</span></div>
            </div>
          </div>
 ...
 <Boxes/>
    <p>{e}</p>
          ...
          <div className="flex space-x-6 pt-9">
            ...
            <button onClick={()=>handelcart(property[0])} className="py-4 text-sm font-bold text-white uppercase bg-teal-500 rounded-sm px-14 hover:bg-teal-600">{added?'Remove':'Add to cart' }</button>
          </div>
